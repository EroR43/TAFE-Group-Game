...
public class GameManager : MonoBehaviour
{
    ...
    // Loads the overworld and checks the parameter of succes is true
    public void LoadWorld(bool success)
    {
        SceneManager.LoadScene(1);
        if (success == true)
        {
            // if success is true, the battleIndex for every BattleNode in the scene checked for the one with the same value as currBattle, running the CompleteBattle func of that node.
    ...
    {
        // local int of completed set to be 0
        int completed = 0;

        //for (int i = 0; i < battleNodeTable.Count; i++)
        foreach (BattleNode node in FindObjectsOfType<BattleNode>())
        {
            // for every BattleNode component in the scene, a local bool of c is set to be to equal the bool value of each node according to their battleIndex
            bool c = (bool)battleNodeTable[node.battleIndex];
            // each nodes completion status is checked, and if a node is completed the value of the completed int is upped by 1
            if (c == true)
            {
                completed++;
            }
        }
        // if the value of completed is equal to the number of battle nodes in the battleNodeTable, this func returns true
