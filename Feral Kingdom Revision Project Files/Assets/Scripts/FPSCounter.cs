...
public class FPSCounter : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // checks if the value is equal to or less than 0, if it is, setting timer to equal refresh, otherwise setting timer to equal the result of taking timeLapse from timer.
        timer = timer <= 0 ? refresh : timer -= timelapse;

        // checks if the value timer is equal to or less than 0, setting avgFramerate to equal the int value of 1f divided by the value timelapse
        if (timer <= 0) avgFramerate = (int)(1f / timelapse);
        // sets the text of the m_Text ui to the string format of avgFramerate in the format of the display string.
